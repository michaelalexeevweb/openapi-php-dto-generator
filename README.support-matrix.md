...
| `if` / `then` / `else` written on the SCHEMA ITSELF, in runtime / laravel / laravel-data | on a PROPERTY the conditional is emitted and enforced in every mode. At the TOP LEVEL of a component these three modes do not carry it into their constraint map at all, so a rule stated about the object as a whole is accepted unchecked; Symfony and yii3 do enforce it there. Workaround: state the conditional on the property, or nest the object one level. Measured 2026-08-31 in all five modes. The neighbouring object-wide keywords (`minProperties`, `dependentRequired`, `not`) are emitted just as unevenly at that level — only yii3 carries all of them — and their enforcement per mode is NOT yet measured |
