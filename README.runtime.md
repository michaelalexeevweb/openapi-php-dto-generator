...
| a path template disagreeing with its `in: path` parameters | generation warns, three ways: a `{placeholder}` no parameter declares, a parameter no placeholder mentions, and a path parameter not marked `required: true`. The emitted class is unchanged — a path parameter is required whatever the document says — so this catches, for instance, `{?name}` (RFC 6570 query expansion, which `paths` does not use) before the first request does. Since 2.15.35 |
| two parameters sharing a NAME in different locations | both are emitted: the first keeps the plain property name, each later one takes its location as a suffix (`id`, `idQuery`, `idHeader`) and keeps the wire name in its alias, so all of them still read the field `id` from their own source. Since 2.15.34 — before it, the later declaration silently overwrote the earlier |
