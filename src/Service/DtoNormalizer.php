...
final class DtoNormalizer implements DtoNormalizerInterface
{
    ...
    private function buildClassMetaFromNormalizationMap(string $className): ?array
    {
        ...
        foreach ($rawMap as $propertyName => $row) {
            ...
            // Reflection on the getter docblock is a fallback only for DTOs generated before the
            // key existed (and the hand-written / public-getter path).
            $arrayItemTypeNames = array_key_exists('arrayItemType', $metadataArr) && is_string($metadataArr['arrayItemType'])
                ? $this->extractArrayItemTypeNames($className, $metadataArr['arrayItemType'])
